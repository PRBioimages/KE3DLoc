...
class ProteinDataset(Dataset):
    def __init__(self, csv_file, transform=None, root=None):
        ...
        self.root = root
        self.label_mapping = {uniprot_id: idx for idx, uniprot_id in enumerate(np.unique(self.data['uniprot_id']))}
    ...
    def __getitem__(self, idx):
        ...
        ids = self.label_mapping[id]
        ID_weight = np.zeros(1).astype(np.float32)  # ID loss is calculated only for the corresponding category
        for loc_tuple in location:
            if loc_tuple[0] in [1,4,6,7,8,9,14,15,16] and loc_tuple[1] == 3:
                ID_weight = np.ones(1).astype(np.float32)
                break
        return torch.tensor(image), torch.tensor(label1), torch.tensor(label2), torch.tensor(ids), torch.tensor(ID_weight)
...
def main():
    ...
    criterion2 = ASLSingleLabel(gamma_neg=4, gamma_pos=0, eps=0.1, reduction=1)
    save_path = '../model_6block_segblock_12_loss/ID_w-4-0-0.1-select'
    ...
    for epoch in range(max_epoch+1, Epochs+1):
        ...
        for inputData, intensity, target, protein_id, id_weight in tqdm(train_dataloader):
            inputData = inputData.cuda()
            intensity = intensity.cuda()
            target = target.cuda()
            protein_id = protein_id.cuda()
            id_weight = id_weight.cuda()
            id_weight = torch.squeeze(id_weight)
            optimizer.zero_grad()
            output, id = model(inputData)
            loss1 = criterion(output, target, intensity)
            loss2 = criterion2(id, protein_id) * id_weight
