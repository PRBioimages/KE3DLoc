...
def main():
    ...
    if if_load == 1:
        ...
        if max_epoch == 0:
            # # load encoder trained based on https://github.com/AllenCellModeling/pytorch_integrated_cell
    ...
    for epoch in range(max_epoch+1, Epochs+1):
        ...
        model.train()
        # model.check_freeze_status()
        # for i, (inputData, intensity, target) in enumerate(tqdm(itertools.islice(train_dataloader, 3))):
