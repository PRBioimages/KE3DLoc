...
def main():
    ...
    save_path = '../true_split/new4-protein-go/compleEx-1gg-1pg-connect-768+768-lianhe'
    ...
    Kmodel = KGEModel(model_name='ComplEx', gamma=12.0, hidden_dim=hidden_dim)
        ...
        for i, (inputData, intensity, target, onto_id) in enumerate(tqdm(train_dataloader)):
            ...
            for j in range(onto_id.shape[0]):
                if onto_id[j] != 78962 and onto_id[j] != 214162:
                    ...
                    negative_tail_go = protein_go_ids[onto_id[j].item()]['negative']['tail_input_ids'].view(12,
                                                                                                            -1)  
            ...
            for j in range(I, I + 1 + ((i - 4545) > 0)):  
                ...
                negative_tail_go = go_go_ids[list(go_go_ids.keys())[j]]['negative']['tail_input_ids'].view(12,
                                                                                                           -1)  
                _, _, _, _, negative_tail_emb = model(tail=negative_tail_go)  # (24,512)
