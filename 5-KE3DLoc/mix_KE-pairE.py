...
def main():
    ...
    hidden_dim = 384 * 2  # Dimensions of Relation
    ...
    go_go_ids = go_go_datas['data']  # id
    protein_go_ids = protein_go_datas['data']
    # used to balance the data,see detail in https://github.com/zjunlp/OntoProtein
    ...
    if if_load == 1:
        if os.path.exists(csv_path):  # make sure to load pretrained model
    ...
    val_dataset = ProteinDataset_val(csv_file='../data_list/opencell_val_data.csv',
                                     raw_protein='../data_list/opencell_val_data_protein.csv',
                                     root='../Data')
    ...
    for epoch in range(max_epoch+1, Epochs+1):
        ...
        for i, (inputData, intensity, target, onto_id) in enumerate(tqdm(train_dataloader)):
            ...
            for j in range(onto_id.shape[0]):
                if onto_id[j] != 78962 and onto_id[j] != 214162:
                    ...
                    _, _, _, _, negative_tail_emb = model(tail=negative_tail_go)
                    ...
                    if if_subsampling_weights == 1:
                        ...
                        ke_loss12 = (ke_loss1 * subsampling_weights).sum() / subsampling_weights.sum()
            ...
            for j in range(I, I + 1 + ((i - 4545) > 0)):  # After a certain limit it becomes two GO-GOs per iteration.
                ...
                negative_tail_go = go_go_ids[list(go_go_ids.keys())[j]]['negative']['tail_input_ids'].view(12, -1)  # 12 is the negative sample size
                _, _, _, _, negative_tail_emb = model(tail=negative_tail_go)
                ...
                if if_subsampling_weights == 1:
                    ...
                    ke_loss12 = ke_loss1.mean()
                    ke_loss22 = ke_loss2.mean()
                loss3 += (ke_loss12 + ke_loss22) * ke_lambda / 2
        ...
        matlab_save_path = os.path.join(save_path, f'data-r-onto-6-{epoch}.mat')
