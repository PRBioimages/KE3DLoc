...
class CombinedModel(nn.Module):
    def __init__(self, enc_3d, enc_2d, model_name, nentity, nrelation, hidden_dim, gamma=None, evaluator=None,
        ...
        self.go_embedding.weight.requires_grad = False  # if freeze go feature
...
def main():
    ...
    if if_load == 1:
        ...
        if max_epoch == 0:
            ...
            model.go_embedding.weight.data.copy_(pretrained_weights)
            print('load go_embedding over')
        elif os.path.exists(model_path):  
            model_state = torch.load(model_path,
                               map_location='cpu')  
            model.load_state_dict(model_state['model_state_dict'], strict=True)  
    ...
    # gcn_model = GCN(input_dim=768, hidden_dim=512, output_dim=384).cuda()
    # gcn_model = GCN(input_dim=768, hidden_dim=1000, output_dim=768).cuda()
    ...
    for epoch in range(max_epoch+1, Epochs+1):
        ...
        # for i in tqdm(go_go_ids):
        #     go_go_data = go_go_ids[i]['postive']
        ...
        #         subsampling_weights = go_go_weights[i]
        ...
        #     loss = (ke_loss12 + ke_loss22) * ke_lambda / 2  # 55
        #     loss.backward()
        ...
        for i, (inputData, intensity, target, onto_id) in enumerate(tqdm(train_dataloader)):
            ...
            head_emb, _, _, _, _ = model(x_3d=inputData)
            H_embedding = []
            for j in range(onto_id.shape[0]):
                if onto_id[j] != 78962 and onto_id[j] != 214162:
                    ...
                    # go_to_idx = {go_id: idx + num_proteins for idx, go_id in enumerate(np.unique(tail_go.cpu().numpy()))}
                    
                    num_gos = len(tail_go)
                    total_nodes = num_proteins + num_gos
                    adj_matrix = torch.eye(total_nodes).cuda()  
                    # for rel, go_id in zip(relation.cpu().numpy(), tail_go.cpu().numpy()):
                    
                    #     go_idx = go_to_idx[go_id]
                    #     adj_matrix[protein_idx, go_idx] = 1  
                    #     adj_matrix[go_idx, protein_idx] = 1
                    ...
                    # print(adj_matrix)  # (29,29)
                    gcn_feature = torch.cat((head_emb[j].unsqueeze(0), tail_emb), dim=0)  # (29,768)
                    # ke_head = gcn_model(gcn_feature, adj_matrix)
                    ke_head = model.GCN(gcn_feature, adj_matrix)  # (1536,)
                    # ke_head = torch.mean(ke_head, dim=0)
                else:
                    # ke_head = torch.stack([head_emb[j], head_emb[j]], dim=0)
                    ke_head = torch.cat([head_emb[j], torch.zeros_like(head_emb[j])], dim=-1)  # (1536,)

                H_embedding.append(ke_head)  # 768*2

            H_embedding = torch.stack(H_embedding)  # (12,1536)
